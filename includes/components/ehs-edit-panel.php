...
if ($karyawan['ws_id'] == 8) {
    ...
    WHERE karyawan_workstation.npk = '{$karyawan['npk']}' AND ehs.name != '' AND workstations.id = '{$karyawan['ws_id']}'
    GROUP BY ehs.name
    ...
    $q_ehs = $conn->query(
        ...
        LEFT JOIN sub_workstations ON sub_workstations.id = ehs.workstation_id
        LEFT JOIN karyawan_workstation ON karyawan_workstation.workstation_id = sub_workstations.id
        WHERE karyawan_workstation.npk = '{$karyawan['npk']}' AND ehs.name != ''
...
while ($p2 = $q_ehs->fetch_assoc()) {
    if ($karyawan['ws_id'] == 8) {
        if ($current_sub_workstation_id != $p2['sub_workstation_id']) {
            
            $current_sub_workstation_id = $p2['sub_workstation_id'];
            echo "<h4>" . $p2['workstation_name'] . "</h4>";
        ...
        echo "<div class='ml-auto mr-4'>
            <input type='text' class='form-control datepicker' name='jadwal_training_ehs-{$p2['ehs_id']}' value='{$formatted_jadwal}' placeholder='yyyy-mm-dd' id='monthYearPickerEhs{$p2['ehs_id']}'></div>";
        echo "
            <input type='checkbox' name='mandatory' class='mandatory-checkbox-ehs' id='mandatory-ehs-" . $p2['ehs_id'] . "' value='{$p2['ehs_mandatory']}'";
        ...
        $(document).ready(function () {
            ...
            $('.mandatory-checkbox-ehs').change(function() {
                ...
                var hiddenInputId = checkboxId.replace('mandatory-ehs', 'mandatory_value-ehs');
