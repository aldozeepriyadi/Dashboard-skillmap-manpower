...
if ($uploadOk == 0) {
    $response['error'] = "Maaf, file Anda tidak diunggah.";
} else {
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
        ...
        foreach ($objPHPExcel->getAllSheets() as $sheetIndex => $sheet) {
            ...
            // Loop melalui setiap baris di Excel mulai dari baris ke-7
            for ($row = 7; $row <= $highestRow; $row++) {
                // Menggunakan getFormattedValue untuk memastikan angka dengan nol di depan dibaca sebagai teks
                ...
                if (!empty($npk) && !empty($workstations_name) && !empty($sub_workstations_name)) {
                    ...
                    if ($sheetTitle == 'Process') {
                        ...
                        $results[] = "Baris $row: Nama lembar kerja tidak dikenal.";
                        continue;
                    }

                    // Cari workstation_id berdasarkan workstation_name dan sub_workstation_name
                    ...
                    if ($workstations_result->num_rows > 0) {
                        $workstation_row = $workstations_result->fetch_assoc();
                        $workstation_id = $workstation_row['id'];
                        ...
                        if ($sub_workstations_result->num_rows > 0) {
                            $sub_workstation_row = $sub_workstations_result->fetch_assoc();
                            $sub_workstation_id = $sub_workstation_row['id'];

                            // Cari process_id berdasarkan process_name
                            $process_result = $conn->query("SELECT $id_column, workstation_id FROM $table_name WHERE UPPER(name) = '$process_name' ");
                            if ($process_result->num_rows > 0) {
                                $process_row = $process_result->fetch_assoc();
                                $process_id = $process_row[$id_column];
                                ...
                                if ($check_result->num_rows > 0) {
                                    ...
                                    if ($conn->query($update_sql) === TRUE) {
                                        ...
                                        $results[] = "Baris $row: Error saat memperbarui data: " . $conn->error;
                                    }
                                } else {
                                    ...
                                    if ($conn->query($insert_sql) === TRUE) {
                                        ...
                                        $results[] = "Baris $row: Error saat memasukkan data: " . $conn->error;
                                    }
                                }
                            } else {
                                $results[] = "Baris $row: Process ID tidak ditemukan untuk Process Name: $process_name.";
                            }
                        } else {
                            $results[] = "Baris $row: Sub Workstation ID tidak ditemukan untuk Sub Workstation Name: $sub_workstations_name.";
                        }
                    } else {
                        $results[] = "Baris $row: Workstation ID tidak ditemukan untuk Workstation Name: $workstations_name.";
                    }
                } else {
        ...
        // Gabungkan hasil ke dalam respon
        $response['results'] = $results;
    } else {
        $response['error'] = "Maaf, terjadi kesalahan saat mengunggah file Anda.";
    }
}
